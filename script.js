...
function estaDesbloqueada(materia) {
  if (["Realidad Nacional", "Inglés"].includes(materia.nombre)) return true;
  const prereq = materias.filter(m => m.desbloquea.includes(materia.nombre));
  if (prereq.length === 0) return materia.ciclo <= 3;
  return prereq.every(m => estado[m.nombre] === "completado");
}
